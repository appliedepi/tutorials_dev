...
Try it out now, type `Sys.Date()`, `Sys.time()`, `today()` and `now()` on different lines and click submit.
...
```{r today_date-check}
  grade_this({
    ...
    fail(message = "Remember to type each of the four functions on a separate line. Also remember date Sys.Date() has Date capitalized")
  })
```
