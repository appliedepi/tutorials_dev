...
question_text(
  "Have you ever had to deal with a dataset that contains multiple data formats? If so, how did you deal with it?",
  answer_fn(function(value) {
    if (grepl(paste(letters, collapse = "|"), value)) {
      correct("")
    }
...
function_order <- c(ymd_hms("2020-01-01 17hrs 5min 6second"),
                    ymd_hm("2020-01-02 16.00"),
                    mdy_hms("January 02 2020, 16:00:40"),
                    dmy_hm("02 January 2020 16:01"))
...
quiz(
  question("Which of the following functions would convert 'Jan 5 1997' to the correct Date format?",
    ...
    incorrect = "All of these would return the date in the correct format!"),
...
question_text(
  "Why does format() return a different order?",
  answer_fn(function(value) {
    if (grepl(paste(letters, collapse = "|"), value)) {
      correct("")
    }
...
question_text(
  "Why would we want to include weeks that have no data?",
  answer_fn(function(value) {
    if (grepl(paste(letters, collapse = "|"), value)) {
      correct("")
    }
...
We would like to now work out the number of `case_outcome` per **month** in a column `monthly_outcome` from the dataset `linelist`. Could you write some code to do this?

```{r epimonth_cases, exercise = TRUE}
linelist %>%

```

```{r epimonth_cases-hint}
```

```{r epimonth_cases-check}
  grade_this({
    # Automatically pass if .result equal to .solution
    pass_if_equal()
    # Default to failing grade with feedback
...
```{r epimonth_cases-solution}
linelist %>% 
  drop_na(date_outcome) %>%             # remove cases missing onset date
  mutate(monthly_outcome = floor_date(   # make new column, week of onset
    date_outcome,
    unit = "month")) %>%            
  count(monthly_outcome) %>%           # group data by week and count rows per group (creates column 'n')
  tidyr::complete(                  # ensure all weeks are present, even those with no cases reported
    monthly_outcome = seq.Date(          # re-define the "weekly_cases" column as a complete sequence,
      from = min(monthly_outcome),       # from the minimum date
      to = max(monthly_outcome),         # to the maxiumum date
      by = "month"),                   # by weeks
    fill = list(n = 0))
...
question_text(
  "What advantages would estimating the missing data bring?",
  answer_fn(function(value) {
    if (grepl(paste(letters, collapse = "|"), value)) {
      correct("")
    }
...
question_text(
  "And what potential pitfalls and things we should be careful of could occur if we estimate dates?",
  answer_fn(function(value) {
    if (grepl(paste(letters, collapse = "|"), value)) {
      correct("")
    }
