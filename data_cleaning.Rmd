...
This allows us to carry out several different alterations of the dataframe in a single operation. It can be helpful to think of the pipe operator `%>%` as telling the computer "and then" between your operational commands. For example, the following code will make use of the **janitor** package to clean the column names (which standardizes capitalization, removes accents and makes sure all names consist of only underscores, numbers and letters) and then display the new names in our console.

```{r, echo = T, eval = F}
linelist %>%          #Load the database, and then
  clean_names() %>%   #Clean the names of the columns, and then
