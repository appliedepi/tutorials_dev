...
To see your plot in a larger window you can click on "Zoom":

We can also remove plots from the plot tab using the red X button to remove the current plot or the little brush to remove all previous plots. 

The "Export" button allows us to save plots to a file or copy to our clipboard to paste elsewhere. To save a `ggplot()` with a function we can use `ggsave()`. This gives more options than exporting via the RStudio window and is also useful to save multiple plots from the same code.
...
In the previous section we saw examples of plots with X and Y and only X plot aesthetics. As well as the simple aesthetics we have already encountered we can also adjust elements specific to the type of plot we are using (the `geom_*()`):
...
If we convert any column in our data to class factor we can adjust the order of the "levels" using the names present. Here we create a new hospital column, hosp_fac with the order of the hospitals changed. 

```{r, echo=T, eval=T}
linelist$hosp_fac <- factor(linelist$hospital, levels = c("Missing", "Other", "Central Hospital", "Military Hospital", "Port Hospital", "St. Mark's Maternity Hospital (SMMH)"), exclude = NULL)          
...
For more simple layouts we can use the **forcats** package to convert to class *factor* and adjust "level" order.  
`fct_infreq()` orders x-axis position and stacks (fill) by frequency:

```{r, eval=T, echo=T}
ggplot(linelist) +             
geom_bar(              
  mapping = aes(
    x = fct_infreq(hospital),   
...
Labels look much improved by using `geom_label_repel()` from the **ggrepel** package. These scale with the size of the point which we have assigned to number of cases in this plot so we should set size = 3 within `geom_label_repel()` to fix this. 
Try changing the following code to use `geom_label_repel()` and `size = 5`:
...
plot_grid(plot1, plot_grid(plot1, plot2, rel_heights = c(1, 1), ncol = 1, align = "hv", labels = c("B", "C")), rel_heights = c(1, 1), ncol = 2, align = "hv", labels = c("A", NA))
```

### Dual axes

You can also use **cowplot** to overlap two plots and create a "dual-axis" plot.  Here we want to use the cowplot default theme which helps us to produce plots with transparent backgrounds.
