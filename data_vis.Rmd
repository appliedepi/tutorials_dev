...
***need to cover static aesthetics and  then go on to mapped aesthetics***

"Aesthetics" are features whose display could vary or each data point. An obvious example are the X and Y co-ordinates but these can also include the colour, shape and other properties of the plot which depend on columns in your data.
...
Here we have set the axes X and Y to columns from the data and you can see this represented on the plot area output from `ggplot`. 
***qu chose a different column to plot and see how the axis labels and values change***
...
We can also plot other simple plots with the same data by switching out the `geom_point()` for another geom. Can you spot what is missing from the following `geom_line()` command which is stopping it from plotting as intended?
